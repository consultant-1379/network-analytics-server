...
function Get-PlatformVersionFile() {
    ...
    $xmlFile = Get-ChildItem $versionDir platform-release.*.xml

    if( $xmlFile -eq $null ) {
        return @($False, "platform-release.xml file not found in $($versionDir)")
    }
...
function Get-PlatformDataFromFile() {
    ...
    if(-not (Test-Path $platformXmlFile)) {
        return @($False, "platform-release.xml not found at path: $($platformXmlFile)")
    }
