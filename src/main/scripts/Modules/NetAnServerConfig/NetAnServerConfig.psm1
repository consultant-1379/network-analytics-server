...
Function Test-UserExists() {
    ...
    $selectQuery = "select * from users where user_name like `'" +  $($map.administrator) + "`'"
...
Function Get-Arguments() {
  ...
  $configArgs = Switch ($key) {
        ...
        set-config-prop-hsts-max-age{
            $logger.logInfo("Using Arguments: 'config set-config-prop -n security.hsts.max-age-seconds -v 31536000'")
            return "config set-config-prop -n security.hsts.max-age-seconds -v 31536000"
        }
		...
		set-config-prop-autoserv-timeout{
            $logger.logInfo("Using Arguments: 'set-config-prop --name=automation-services.job-inactivity-timeout --value=60'")
            return "set-config-prop --name=automation-services.job-inactivity-timeout --value=60"
        }
      ...
      #This is for creating platform admin user
        ...
        update-deployment {
            $allpackages=  $($map.netanserverDeploy)+","+$($map.netanserverBranding)+","+$($map.nodeManagerDeploy)+","+$($map.pythonDeploy)+","+$($map.TERRDeploy)## Removed te hotfix, Added python & TERR deployment
