...
#
# Arguments:
#   [hashtable] $osVersionmap e.g. "Microsoft Windows Server 2012 R2 Standard or Microsoft Windows Server 2016 Standard"
#
# Return Values:
#   [boolean]$true|$false
#
...
function Test-OS {
    ...
    if ($osCaption.contains($osVersionmap.osVersion2016) -or  $osCaption.contains($osVersionmap.osVersion2019) ){
       return $True 
    } else {
        return $False
    }
...
#   $_.Release -ge 379893 corresponds to 4.5.2
#   Test the OS Version.
#
# Return Values:
#   [boolean]$true|$false
...
function Test-FrameWork {
    $a = Get-ChildItem 'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP' -recurse |
		 Get-ItemProperty -name Version,Release -EA 0 |Where { $_.PSChildName -match '^(?!S)\p{L}' -and $_.Release -ge 379893} 
    if ($a.Count -gt 0) {
       return $True 
    } else {
        return $False
