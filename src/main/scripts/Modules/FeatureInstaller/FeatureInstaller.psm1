...
Function Install-Feature() {
	...
	if ($featureName -eq "PM-Explorer") {
		...
		{
			Write-Host("Business Analyst Group Permissions Missing ::")
			...
			$_.InputObject
				...
				if($_.InputObject -eq "READ")
				{
					...
					$logger.logInfo("Read Permission Added", $True)
				}
				elseif($_.InputObject -eq "WRITE")
				{
					...
					$obj = Invoke-UtilitiesSQL -Database $DB -Username $USER -Password $password -ServerInstance $SERVER -Query $query -Action fetch
					$logger.logInfo("Write Permission Added", $True)
				}
				elseif($_.InputObject -eq "EXECUTE")
				{
					...
					$obj = Invoke-UtilitiesSQL -Database $DB -Username $USER -Password $password -ServerInstance $SERVER -Query $query -Action fetch
					$logger.logInfo("Execute Permission Added", $True)
				}
			}
		}
		$logger.logInfo("Permissions Set for Business Analyst Group", $True)
		
		$logger.logInfo("Checking Permissions which exist for Business Author Group", $True)
		...
		{
			Write-Host("Business Author Group Permissions Missing ::")
		...
		$logger.logInfo("Permissions Set for Business Author Group", $True)
		
		$logger.logInfo("Checking Permissions which exist for Everyone Group", $True)
		$search="Everyone"
		...
		{
			Write-Host("Everyone Group Permissions Missing ::")
		...
		$logger.logInfo("Permissions Set for Everyone Group", $True)
...
Function Set-Parent(){
    ...
    $query = "update lib_items set parent_id='$parentGUID' where item_id='$childGUID'" 
